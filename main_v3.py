...
from model import Style2VecV3, NegLossV2, Normalize
...
def train(model, loader):
    ...
    for epoch in range(1, 5+1):
        ...
        model_save_name = 'Style2vecV2_B0_invert_context_cnn_num_train_layer_{}_emb_dim_{}_neg_{}_epoch_{}.pt'.format(num_train_layer, 512, 5, epoch)
        path = "./trained_model/{}".format(model_save_name)
        torch.save(model.state_dict(), path)
    elapsed_train_time = time.time() - train_start
    print('Finished training. Train time was:', elapsed_train_time)


model = Style2VecV3(num_train_layer=num_train_layer)
